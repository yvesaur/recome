...
router.get("/api/v1/news", async (req, res) => {
  try {
    const response = await db.query(
      "SELECT * FROM news WHERE DATE(date) = DATE('2023-12-19') ORDER BY date DESC LIMIT 50"
    );
...
router.get("/api/v1/fetchNews", async (req, res) => {
  try {
    const response = await db.query(
      "SELECT * FROM news WHERE DATE(date) = DATE('2023-12-19') ORDER BY date DESC LIMIT 3000"
    );
