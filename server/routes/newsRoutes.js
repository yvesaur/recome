...
router.get("/api/v1/news", async (req, res) => {
  try {
    const response = await db.query(
      "SELECT * FROM news WHERE DATE(date) <= DATE('2024-01-26') ORDER BY date DESC LIMIT 50"
    );
...
router.get("/api/v1/fetchNews", async (req, res) => {
  try {
    const response = await db.query(
      "SELECT * FROM news WHERE DATE(date) <= DATE('2024-01-26') ORDER BY date DESC LIMIT 5000"
    );
